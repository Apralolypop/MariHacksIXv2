...
def f1(x, t):
    k = 1
    #x'(t) = cx(t)
    return np.array([1 / (1 + t ** 2)]) # x[1] is velocity, x[0] is position

def f2(t):
    return np.array([math.atan(t)]) # real results for exponential growth
...
ax.set_title('Standard Deviation vs N for exponential function from 0 to 100 t')
ax.set_xlabel('N')
